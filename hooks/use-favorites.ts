...
export function useFavorites() {
  ...
  const has = useCallback(
    (complexId: string) =>
      favorites.some((favorite) => favorite.id === complexId),
    [favorites],
  );
  const toggle = useCallback(
    async (
      ...
      setFavorites((current) =>
        saved
          ? current.filter((item) => item.id !== complex.id)
          : [complex, ...current],
      );
      try {
        const response = await fetch(
          saved
            ? `/api/buyer/favorites?complexId=${encodeURIComponent(complex.id)}`
            : '/api/buyer/favorites',
          saved
            ? { method: 'DELETE' }
            : {
