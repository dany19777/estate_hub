              ...
              <div className="listing-stack">
                {inventory.map((listing) => (
                  <article className="listing-row" key={listing.id}>
                    ...
                    <div className="listing-info"><div>{listing.promoted && <Badge className="sponsored-badge"><Sparkles /> {listing.sponsoredLabel}</Badge>}<Badge variant={listing.marketType === 'PRIMARY_DEVELOPER' ? 'default' : 'secondary'}>{listing.marketType === 'PRIMARY_DEVELOPER' ? 'Первичный' : 'Вторичный'}</Badge></div><h3>{listing.rooms}-комнатная квартира, {listing.areaSqm} м²</h3><p>{listing.floorNumber} / {listing.totalFloors} этаж · {listing.finish}</p><span><ShieldCheck /> {listing.seller}</span></div>
