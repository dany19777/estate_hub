...
export default function BuyerProfile() {
  ...
  const activeDispute = disputes.find((item) => item.reservation_id === activeReservation?.id && ['open', 'in_review'].includes(item.status));
        ...
        <section className="profile-content">
          ...
          <div className="profile-main-grid">
            ...
            <aside className="profile-right-rail">
              ...
              <section className="preference-card"><span>Ваша AI-подборка</span><h2>{recommendations.basis?.label ?? 'Проверенные предложения'}</h2><p>{recommendations.loading ? 'Обновляем по актуальному каталогу…' : `${recommendations.recommendations.length} подходящих ЖК по вашим критериям`}</p><div><span><Sparkles /></span><p><strong>Критерии прозрачны</strong><small>Причины показаны у каждого объекта</small></p></div><button type="button" onClick={() => setActive('Рекомендации')}>Открыть рекомендации <ChevronRight /></button></section>
            </aside>
          </div>
        </section>
      </div>
