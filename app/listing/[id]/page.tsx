...
export default function ListingPage() {
  ...
  return <main className="listing-detail-page">
    ...
    <div className="shell listing-detail-shell">
      ...
      <aside className="listing-detail-rail"><section className="listing-purchase-card"><small>Цена квартиры</small><strong>{formatPriceMillions(listing.priceUzs)} сум</strong><span>{formatPricePerSqm(Math.round(listing.priceUzs/listing.areaSqm))}</span><div><Banknote/><p><b>Цена из реестра</b><em>Обновлена {formatDate(priceHistory.at(-1)?.changedAt ?? listing.publishedAt)}</em></p></div>{listing.reserveEnabled ? <ReservationDialog listingId={listing.id} unitNumber={listing.unitNumber} trigger={<Button className="listing-primary-action"/>}/> : secondary ? <ChatDialog listingId={listing.id} complexName={complex.name} unitNumber={listing.unitNumber} seller={listing.seller} trigger={<Button className="listing-primary-action"><MessageCircle/> Написать продавцу</Button>}/> : <LeadRequestDialog type="consultation" complexId={complex.id} complexName={complex.name} listingId={listing.id} unitNumber={listing.unitNumber} trigger={<Button className="listing-primary-action"><MessageCircle/> Получить консультацию</Button>}/>} {secondary && listing.contactPhone && <a className="listing-call-action" href={`tel:${listing.contactPhone}`}><Phone/> {listing.contactPhone}</a>}<p className="listing-flow-note">{secondary ? 'Для вторичного рынка онлайн-бронирование недоступно: условия просмотра и сделки согласуются с продавцом.' : 'Бронь доступна только у застройщика и подтверждается отдельной оплатой.'}</p></section>
