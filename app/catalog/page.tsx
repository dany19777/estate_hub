...
const marketValue = (market: string): 'all' | 'primary' | 'secondary' =>
  market === 'Первичный'
    ? 'primary'
    : market === 'Вторичный'
      ? 'secondary'
...
export default function CatalogPage() {
  ...
  const { currency, usdUzs } = useBuyerPreferences();
  ...
  const [activeMarket, setActiveMarket] = useState(() =>
    initialMarket === 'primary'
      ? 'Первичный'
      : initialMarket === 'secondary'
        ? 'Вторичный'
        : 'Все',
  );
  ...
  const [city, setCity] = useState(
    () => searchParams.get('city') ?? 'Самарканд',
  );
  const [complexSlug, setComplexSlug] = useState(
    () => searchParams.get('complex') ?? '',
  );
  const [rooms, setRooms] = useState<number | undefined>(() =>
    numberParam(searchParams.get('rooms')),
  );
  const [status, setStatus] = useState<
    'completed' | 'under_construction' | undefined
  >(() =>
    initialStatus === 'completed' || initialStatus === 'under_construction'
      ? initialStatus
      : undefined,
  );
  const [seller, setSeller] = useState<SellerType | undefined>(() =>
    initialSeller === 'developer' ||
    initialSeller === 'owner' ||
    initialSeller === 'agency'
      ? initialSeller
      : undefined,
  );
  const [minPrice, setMinPrice] = useState(() =>
    numberParam(searchParams.get('minPrice'))
      ? String(Number(searchParams.get('minPrice')) / 1_000_000)
      : '',
  );
  const [maxPrice, setMaxPrice] = useState(() =>
    numberParam(searchParams.get('maxPrice'))
      ? String(Number(searchParams.get('maxPrice')) / 1_000_000)
      : '',
  );
  ...
  const [removedAiFilters, setRemovedAiFilters] = useState<string[]>(() =>
    (searchParams.get('excludeParsed') ?? '').split(',').filter(Boolean),
  );
  ...
  const [sort, setSort] = useState<CatalogSort>(() =>
    ...
    initialSort === 'price_per_sqm' ||
    initialSort === 'newest' ||
    initialSort === 'area_desc'
      ? initialSort
      : 'recommended',
  );
  ...
  const request = useMemo(
    () => ({
      ...
      seller,
      minPrice: minPrice
        ? Number(minPrice) * (currency === 'USD' && usdUzs ? usdUzs : 1_000_000)
        : undefined,
      maxPrice: maxPrice
        ? Number(maxPrice) * (currency === 'USD' && usdUzs ? usdUzs : 1_000_000)
        : undefined,
  ...
  const activeSelection =
    filtered.find((item) => item.id === selected?.id) ?? filtered[0] ?? null;
  ...
  useEffect(() => {
    ...
    if (minPrice)
      params.set(
        ...
        String(
          Math.round(
            Number(minPrice) *
              (currency === 'USD' && usdUzs ? usdUzs : 1_000_000),
          ),
        ),
      );
    if (maxPrice)
    ...
    if (removedAiFilters.length)
      params.set('excludeParsed', removedAiFilters.join(','));
  ...
  const saveSearch = async () => {
    ...
    const name =
      search.trim() ||
      `${activeMarket}: ${rooms ? `${rooms} комн.` : 'все квартиры'}`;
    const response = await fetch('/api/buyer/preferences', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, filters: request }),
    });
    const payload = (await response.json()) as { message?: string };
    setSaveMessage(
      response.ok
        ? 'Поиск сохранён'
        : (payload.message ?? 'Не удалось сохранить'),
    );
  };
    ...
    <main className="catalog-page">
      ...
      <section className="catalog-toolbar">
        <div className="catalog-search">
          ...
          <Input
            ...
            placeholder="ЖК, район или запрос на обычном языке"
        ...
        <div className="market-quick-tabs" aria-label="Тип рынка">
          {['Все', 'Первичный', 'Вторичный'].map((item) => (
            <button
              type="button"
      ...
      <div className="catalog-layout">
        <aside className={`filter-sidebar ${filtersOpen ? 'mobile-open' : ''}`}>
          ...
          <div className="active-ai-filter">
            <Bot />
            <div>
              <span>{search ? 'AI понял ваш запрос' : 'AI-поиск готов'}</span>
              {search ? (
                <div className="ai-filter-chips">
                  {catalog.parsedFilters.length ? (
                    catalog.parsedFilters.map((filter) => (
                      <button
                        type="button"
                        key={filter.key}
                        onClick={() =>
                          setRemovedAiFilters((current) => [
                            ...current,
                            filter.key,
                          ])
                        }
                    ...
                    <strong>Поиск: {search}</strong>
                  )}
                </div>
              ) : (
                <strong>Опишите квартиру обычным языком</strong>
              )}
              {catalog.unsupportedCriteria.length > 0 && (
                <small>
                  Пока не учитываем: {catalog.unsupportedCriteria.join(', ')}
                </small>
              )}
              {catalog.validationWarnings.map((warning) => (
                <small className="ai-validation-warning" key={warning}>
                  {warning}
                </small>
              ))}
            </div>
          ...
          <div className="price-filter">
            <span className="price-filter-label">Цена, {currency}</span>
            <div>
              <Input
                inputMode="numeric"
                value={minPrice}
                ...
                placeholder={currency === 'USD' ? 'от 40 000' : 'от 450 млн'}
                aria-label={`Минимальная цена, ${currency}`}
              />
              <Input
                inputMode="numeric"
                value={maxPrice}
                ...
                placeholder={currency === 'USD' ? 'до 80 000' : 'до 900 млн'}
                aria-label={`Максимальная цена, ${currency}`}
              />
            </div>
            <span>
              <i />
          ...
          <fieldset className="filter-group">
            ...
            <div>
              <button
                className={rooms === undefined ? 'active' : ''}
                type="button"
          ...
          <fieldset className="filter-group">
            ...
            <div>
              ...
              <button
                className={status === 'under_construction' ? 'active' : ''}
                type="button"
          ...
          <fieldset className="filter-group">
            ...
            <div>
              <button
                className={seller === undefined ? 'active' : ''}
                type="button"
                onClick={() => setSeller(undefined)}
              >
                Все
              </button>
              <button
                className={seller === 'developer' ? 'active' : ''}
                type="button"
                onClick={() => setSeller('developer')}
              >
                Застройщик
              </button>
              <button
                className={seller === 'owner' ? 'active' : ''}
                type="button"
              ...
              <button
                className={seller === 'agency' ? 'active' : ''}
                type="button"
                onClick={() => setSeller('agency')}
              >
                Агентство
              </button>
            </div>
          ...
          <fieldset className="filter-group filter-input-group">
            ...
            >
              ...
              {catalog.facets.cities.map((item) => (
            ...
            >
              ...
              {catalog.facets.districts.map((item) => (
                <option key={item}>{item}</option>
              ))}
            </select>
            <select
              value={complexSlug}
              onChange={(event) => setComplexSlug(event.target.value)}
              aria-label="Жилой комплекс"
            >
              <option value="">Все жилые комплексы</option>
              {catalog.facets.complexes.map((item) => (
          ...
          <fieldset className="filter-group filter-input-group">
            ...
            <div>
              <Input
                ...
                value={minArea}
                onChange={(event) =>
                  setMinArea(event.target.value.replace(/[^\d.]/g, ''))
                }
                placeholder="от"
              />
              <Input
                ...
                value={maxArea}
                onChange={(event) =>
                  setMaxArea(event.target.value.replace(/[^\d.]/g, ''))
                }
                placeholder="до"
              />
            </div>
          ...
          <fieldset className="filter-group filter-input-group">
            ...
            <div>
              <Input
                ...
                value={minFloor}
                onChange={(event) =>
                  setMinFloor(event.target.value.replace(/\D/g, ''))
                }
                placeholder="от"
              />
              <Input
                ...
                value={maxFloor}
                onChange={(event) =>
                  setMaxFloor(event.target.value.replace(/\D/g, ''))
                }
                placeholder="до"
              />
            </div>
        ...
        <section className="catalog-results">
          <div className="catalog-results-heading">
            <div>
              ...
              <h1>
                {loading
                  ? 'Ищем предложения…'
                  : `${catalog.total} жилых комплексов`}
              </h1>
              {saveMessage && (
                <small className="saved-search-message">{saveMessage}</small>
              )}
            </div>
            ...
            <label className="catalog-sort">
              ...
              >
                ...
                <option value="area_desc">Сначала больше площадь</option>
              </select>
            </label>
          </div>
          ...
          <section
            ...
            aria-label={`Примеры раздела ${context.label}`}
          >
            <div>
          ...
          {loading ? (
            <output className="catalog-state">
              ...
              <span>
                <strong>Проверяем актуальные объявления</strong>
                <small>
            ...
            <section className="ai-alternatives">
              <div className="ai-alternatives-heading">
                <span>
                  <Sparkles />
                </span>
                <div>
                  <strong>Точных совпадений нет</strong>
                  <p>
                    {catalog.alternativeReason ??
              ...
              {filtered.map((item) => (
                <article className="result-card" key={item.id}>
                  <div className="result-card-image">
                    ...
                    {item.sponsored ? (
                      <Badge
                        className={`sponsored-badge ${item.specialOffer ? 'special-offer-badge' : ''}`}
                      >
                        <Sparkles /> {item.sponsoredLabel}
                      </Badge>
                    ) : (
                      <Badge>{marketLabel(item.marketTypes)}</Badge>
                    )}
                    <button
                      className={isFavorite(item.id) ? 'active' : ''}
                      onClick={() =>
                        void toggleFavorite({
                          id: item.id,
                          slug: item.slug,
                          ...
                          completion_label: item.completionLabel,
                        })
                      }
                      type="button"
                  ...
                  >
                    <div className="result-title-row">
                      <div>
                        <h2>{item.name}</h2>
                        <p>
                          <MapPin /> {item.city}, {item.district}
                        </p>
                      </div>
            ...
            <div className="map-surface real-map-surface">
              ...
              <div className="map-result-count">
                ...
                <span>ЖК по выбранным фильтрам</span>
              </div>
              {activeSelection && (
                <div className="map-card">
                  ...
                  <div>
                    <Badge>
                      {activeSelection.specialOffer
                        ? activeSelection.specialOfferLabel
                        : marketLabel(activeSelection.marketTypes)}
                    </Badge>
                    <h2>{activeSelection.name}</h2>
                    <p>
                      <MapPin /> {activeSelection.city},{' '}
                      {activeSelection.district}
                    </p>
                    <p className="map-developer">
                      <ShieldCheck />{' '}
                      {activeSelection.developerVerified
