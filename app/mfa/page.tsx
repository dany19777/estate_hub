...
export default function MfaPage() {
  ...
  return <main className="login-page"><section className="login-card">
    ...
    {(state?.status === 'code_required' || secret) && !recoveryCodes.length && <form onSubmit={verify}>
      <label htmlFor="mfa-code">{recoveryMode ? 'Одноразовый резервный код' : 'Шестизначный код из приложения-аутентификатора'}</label>
    ...
    {state?.status === 'code_required' && !recoveryCodes.length && <Button type="button" variant="outline" onClick={() => { setRecoveryMode(!recoveryMode); setCode(''); setError(''); }}>{recoveryMode ? 'Ввести код из приложения' : 'Использовать резервный код'}</Button>}
