...
const adminNav = [
  ...
  { label: 'Роли и доступ', icon: ShieldCheck },
...
export default function AdminDashboard() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      const payload = (await verificationResponse.json()) as Omit<
        AdminDashboardData,
        'moderation'
      > & { message?: string };
      const moderationPayload = (await moderationResponse.json()) as {
        queue?: AdminDashboardData['moderation'];
        message?: string;
      };
      if (!verificationResponse.ok)
        throw new Error(
          payload.message || 'Не удалось загрузить административный кабинет.',
        );
      if (!moderationResponse.ok)
        throw new Error(
          moderationPayload.message ||
      ...
      setLoadError(
        error instanceof Error
          ? error.message
          : 'Не удалось загрузить административный кабинет.',
      );
    }
  }
  ...
  const visibleCases = useMemo(() => {
    ...
    return (dashboard?.queue ?? []).filter((item) => {
      ...
      const matchesTab =
        queue === 'Все' ||
        (queue === 'Новые' && status === 'Новая') ||
        (queue === 'В работе' && status === 'В работе') ||
        (queue === 'Эскалации' && item.risk_level === 'high');
      return (
        matchesTab &&
        (!normalizedQuery ||
          `${item.applicant} ${item.subject_id}`
            .toLowerCase()
  ...
  async function decide(
    ...
    if (
      decision === 'reject' &&
      ['buyer', 'listing'].includes(verification.subject_type)
    ) {
      reason =
        window
          .prompt(
            verification.subject_type === 'listing'
              ? 'Укажите продавцу причину отказа'
              : 'Укажите причину отказа покупателю',
          )
          ?.trim() ?? '';
      if (!reason) return;
    } else if (
      decision === 'reject' &&
      !window.confirm('Отклонить эту заявку? Проект не попадёт в каталог.')
    )
      return;
    ...
    try {
      const response = await fetch('/api/admin/verifications', {
        ...
        body: JSON.stringify({ caseId: verification.id, decision, reason }),
      });
  ...
  async function moderate(complexId: string, decision: 'publish' | 'reject') {
    if (
      decision === 'reject' &&
      !window.confirm('Отклонить материалы на модерации?')
    )
      return;
    ...
    try {
      const response = await fetch('/api/admin/moderation', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ complexId, decision }),
      });
      const payload = (await response.json()) as { message?: string };
      if (!response.ok)
        throw new Error(
          payload.message || 'Не удалось сохранить решение модерации.',
        );
      setFeedback(
        payload.message ||
          (decision === 'publish'
      ...
      setFeedback(
        error instanceof Error
          ? error.message
          : 'Не удалось сохранить решение модерации.',
      );
    } finally {
      setProcessing('');
    }
  ...
  const userInitials = (dashboard?.session.user.fullName ?? 'Администратор')
    .split(/\s+/)
    .slice(0, 2)
    .map((part) => part[0])
    .join('')
    ...
    <main className="platform-admin dark">
      <aside className={`admin-sidebar ${mobileNav ? 'open' : ''}`}>
        ...
        <div className="admin-user">
          ...
          <div>
            <strong>
              {dashboard?.session.user.fullName ?? 'Администратор'}
            </strong>
            <small>
              {dashboard?.session.platformRoles[0] ?? 'Platform Admin'}
            </small>
          </div>
          <ChevronDown />
        </div>
          ...
          {adminNav.map((item) => {
            const count =
              item.label === 'Верификация'
                ...
                : item.label === 'Модерация'
                  ? dashboard?.moderation.length
                  : item.label === 'Отзывы'
                    ? reviews.stats.pending + reviews.stats.reports
                    : item.label === 'Брони и платежи'
                      ? finance.stats.operationsCount
                      : item.label === 'Споры и возвраты'
                        ? disputes.stats.active
                        : item.label === 'Тарифы и биллинг'
                          ? billing.stats.activeSubscriptions
                          : item.label === 'Продвижение'
                            ? promotions.stats.active +
                              promotions.stats.scheduled
                            : item.count;
            return (
              <button
                type="button"
                className={activeNav === item.label ? 'active' : ''}
                onClick={() => navigateAdmin(item.label)}
                key={item.label}
              >
                <item.icon />
      ...
      <section className="admin-workspace">
        <header className="admin-topbar">
          ...
          <div className="admin-global-search">
            ...
            <Input
              ...
              placeholder="Компания или объект…"
            />
          </div>
          <div>
            <button
        ...
        >
          ...
          <div className="admin-kpis">
            <article>
              <span className="ak-blue">
                <Users />
              </span>
              <div>
                <small>Пользователи</small>
                <strong>{dashboard?.stats.users ?? '—'}</strong>
                <em>активные профили</em>
              </div>
            </article>
            <article>
              <span className="ak-green">
                <Building2 />
              </span>
              <div>
                <small>Активные ЖК</small>
                <strong>{dashboard?.stats.activeComplexes ?? '—'}</strong>
                <em>{dashboard?.stats.publishedListings ?? 0} объявлений</em>
              </div>
            </article>
            <article>
              <span className="ak-orange">
                <FileCheck2 />
              </span>
              <div>
                <small>На верификации</small>
                <strong>{dashboard?.stats.pendingVerifications ?? '—'}</strong>
                <em>в текущей очереди</em>
              </div>
            </article>
            <article>
              <span className="ak-violet">
                <WalletCards />
              </span>
              <div>
                <small>Опубликовано</small>
                <strong>{dashboard?.stats.publishedListings ?? '—'}</strong>
                <em>активных объявлений</em>
              </div>
            </article>
            <article>
              <span className="ak-red">
                <AlertTriangle />
              </span>
              <div>
          ...
          <div className="admin-main-grid">
            <section
              className="admin-panel verification-queue"
              id="verification"
            >
              <div className="admin-panel-heading">
                <div>
                  <h2>Очередь верификации</h2>
                  <p>Покупатели, компании и жилые комплексы</p>
                </div>
                <span className="queue-total">
                  {dashboard?.stats.pendingVerifications ?? 0} заявок
                </span>
              </div>
              <div className="admin-queue-toolbar">
                <div>
                  {['Все', 'Новые', 'В работе', 'Эскалации'].map((item) => (
                    <button
              ...
              <Table className="admin-table">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Предмет проверки</TableHead>
                ...
                <TableBody>
                  ...
                  {dashboard && visibleCases.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={7}>
                        <div className="table-empty-state">
                          В этой части очереди заявок нет.
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
                  {visibleCases.map((item) => {
                    ...
                    const status = queueLabels[item.status] ?? item.status;
                    const type =
                      item.subject_type === 'buyer'
                        ? 'Покупатель'
                        : item.subject_type === 'listing'
                          ? 'Продавец вторички'
                          : item.subject_type === 'complex'
                            ? 'Жилой комплекс'
                            : item.organization_type === 'agency'
                              ? 'Агентство'
                    ...
                    const subject =
                      item.subject_type === 'buyer'
                        ? `${item.document_type === 'passport' ? 'Паспорт' : 'ID-карта'} и возраст`
                        : item.subject_type === 'listing'
                          ? item.document_type === 'power_of_attorney'
                            ? 'Доверенность и полномочия'
                            : 'Право собственности'
                          : item.subject_type === 'complex'
                            ? 'Объект и связь с застройщиком'
                            : 'Компания и полномочия';
                      ...
                      <TableRow key={item.id}>
                        <TableCell>
                          <div className="admin-applicant">
                            <span>
                              {item.applicant.slice(0, 2).toUpperCase()}
                            </span>
                        ...
                        <TableCell>
                          <Badge
                            className={`queue-status ${status === 'Новая' ? 'new' : 'working'}`}
                          >
                            {status}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <div className="verification-actions">
                            <button
                              ...
                              aria-label={`Одобрить ${item.applicant}`}
                              title="Одобрить"
                            >
                              <Check />
                            </button>
                            <button
                              ...
                              disabled={processing === item.id}
                              aria-label={`Отклонить ${item.applicant}`}
                              title="Отклонить"
                            >
                              <X />
                            </button>
            ...
            <aside className="admin-side-stack">
              <section className="admin-panel attention-panel">
                ...
                <article>
                  ...
                  <div>
                    <strong>{disputes.stats.active} активных споров</strong>
                    <small>
                      {disputes.stats.highPriority} с высоким приоритетом
                    </small>
                  </div>
                  <ChevronDown />
                </article>
                <article>
                  ...
                  <div>
                    <strong>{highRiskCount} проверок высокого риска</strong>
                    <small>Требуют приоритетного решения</small>
                  </div>
                  <ChevronDown />
                </article>
                <article>
                  ...
                  <div>
                    <strong>
                      {finance.stats.reviewCount} операций на сверке
                    </strong>
                    <small>Провайдер и резерв должны совпадать</small>
                  </div>
                  <ChevronDown />
                </article>
              </section>
              <section className="admin-panel finance-summary">
                <div className="admin-panel-heading">
                  <div>
                    <h2>Финансовый контур</h2>
                    <p>Весь журнал</p>
                  </div>
                </div>
                <div>
                  <span>
                    <small>Оплаты броней</small>
                    <strong>{formatAdminMoney(finance.stats.totalPaid)}</strong>
                  </span>
                  <span>
                    <small>Возвращено</small>
                    <strong>
                      {formatAdminMoney(finance.stats.totalRefunded)}
                    </strong>
                  </span>
                  <span>
                    <small>На сверке</small>
                    <strong>{finance.stats.reviewCount} операций</strong>
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => navigateAdmin('Брони и платежи')}
                >
                  Открыть операции
                </button>
              </section>
          ...
          <section
            className="admin-panel moderation-queue-panel"
            id="moderation"
          >
            <div className="admin-panel-heading">
              <div>
                <h2>Очередь модерации публикаций</h2>
                <p>Проверенные ЖК и новые объявления перед выходом в каталог</p>
              </div>
              <span className="queue-total">
                {dashboard?.moderation.length ?? 0} заявок
              </span>
            </div>
            <Table className="admin-table">
              <TableHeader>
                <TableRow>
                  <TableHead>Жилой комплекс</TableHead>
                  <TableHead>Застройщик</TableHead>
                  <TableHead>Этап</TableHead>
                  <TableHead>Новых объявлений</TableHead>
                  <TableHead>Решение</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                ...
                {dashboard && dashboard.moderation.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5}>
                      <div className="table-empty-state">
                ...
                {dashboard?.moderation.map((item) => (
                  <TableRow key={item.complex_id}>
                    ...
                    <TableCell>
                      <Badge className="queue-status working">
                        {item.workflow_status === 'pending_moderation'
                          ? 'Публикация ЖК'
                          : 'Новый инвентарь'}
                      </Badge>
                    </TableCell>
                    <TableCell>{item.pending_listings}</TableCell>
                    <TableCell>
                      <div className="verification-actions">
                        <button
                          type="button"
                          className="approve"
                          onClick={() =>
                            void moderate(item.complex_id, 'publish')
                          }
                          disabled={processing === item.complex_id}
                          aria-label={`Опубликовать ${item.name}`}
                          title="Опубликовать"
                        >
                          <Check />
                        </button>
                        <button
                          ...
                          className="reject"
                          onClick={() =>
                            void moderate(item.complex_id, 'reject')
                          }
                          disabled={processing === item.complex_id}
          ...
          <AdminDisputesPanel
            ...
            onDecision={async (disputeId, action, note) => {
              const message = await disputes.decide(disputeId, action, note);
              setFeedback(message);
          ...
          <AdminBillingPanel
            ...
            onUpdatePlan={async (plan) => {
              const message = await billing.updatePlan(plan);
              setFeedback(message);
            }}
            onUpdateConfig={async (feeUzs, periodDays) => {
          ...
          <AdminPromotionsPanel
            ...
            onUpdateProduct={async (product) => {
              const message = await promotions.updateProduct(product);
              setFeedback(message);
            }}
            onCancel={async (promotionId) => {
            ...
            <section className="admin-panel system-health" id="system-health">
              ...
              <div>
                ...
                <p>
                  ...
                  <strong>{finance.stats.operationsCount}</strong>
                </p>
              </div>
            </section>
          </div>
