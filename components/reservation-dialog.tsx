...
export function ReservationDialog({ listingId, unitNumber, trigger }: { listingId: string; unitNumber: string; trigger: ReactElement }) {
  ...
  const createHold = async (event: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await response.json() as { reservationId?: string; holdExpiresAt?: string; message?: string };
      if (!response.ok || !data.reservationId) throw new Error(data.message ?? 'Не удалось удержать квартиру.');
  ...
  const reset = () => { setStage('form'); setReservationId(''); setHoldExpiresAt(''); setReservationExpiresAt(''); setError(''); createKey.current = ''; confirmKey.current = ''; };
