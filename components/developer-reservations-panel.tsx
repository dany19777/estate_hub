...
export function DeveloperReservationsPanel({ reservations, stats, loading, error, reload }: { reservations: DeveloperReservation[]; stats: { active: number; holds: number; deals: number; total: number }; loading: boolean; error: string | null; reload: () => Promise<void> }) {
  ...
  async function updateReservation(reservation: DeveloperReservation, action: 'visit_completed' | 'deal_in_progress' | 'extend' | 'buyer_refused' | 'developer_refused') {
    ...
    if (action === 'buyer_refused' && !window.confirm(reservation.payment_reference?.startsWith('LOCAL-DEMO-') ? 'Отменить тестовую бронь по запросу покупателя? Оплаты не было.' : 'Зафиксировать добровольный отказ покупателя? Сумма брони остаётся невозвратной.')) return;
    if (action === 'developer_refused' && !window.confirm(reservation.payment_reference?.startsWith('LOCAL-DEMO-') ? 'Отменить тестовую бронь? Оплаты и возврата нет.' : 'Отменить бронь по вине застройщика и зарегистрировать полный возврат?')) return;
  ...
  return (
    <section className="dashboard-panel developer-reservations-panel" id="developer-reservations">
      <div className="panel-heading"><div><h2>Бронирования</h2><p>Активные брони, тестовый режим и результат визита</p></div><div className="reservation-kpis"><span><strong>{stats.active}</strong> активных</span><span><strong>{stats.holds}</strong> ожидают</span><span><strong>{stats.deals}</strong> в сделке</span></div></div>
