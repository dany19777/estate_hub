...
export function AdminBillingPanel({ plans, subscriptions, config, secondaryListings, events, stats, loading, error, processing, onRetry, onUpdatePlan, onUpdateConfig, onActivateSecondary }: Props) {
  ...
  return (
    <section className="admin-panel admin-billing-panel" id="billing">
      ...
      <div className="admin-billing-block">
        ...
        <Table className="admin-table"><TableHeader><TableRow><TableHead>Компания</TableHead><TableHead>Тариф</TableHead><TableHead>Использование</TableHead><TableHead>Статус</TableHead><TableHead>Оплачен до</TableHead></TableRow></TableHeader><TableBody>
          {loading && <TableRow><TableCell colSpan={5}><div className="table-empty-state">Загружаем подписки…</div></TableCell></TableRow>}
