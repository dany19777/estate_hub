...
export function DeveloperBillingPanel({ plans, subscription, usage, events, loading, error, feedback, processing, onRetry, onChangePlan, onRenew }: Props) {
  ...
  return (
    <section className="dashboard-panel developer-billing-panel" id="developer-billing">
      <div className="panel-heading"><div><h2>Тариф и оплата</h2><p>Лимит активного инвентаря и история списаний</p></div><Badge className={`billing-subscription-state ${subscription?.status ?? 'past_due'}`}>{subscription?.status === 'trialing' ? 'Пробный период' : subscription?.status === 'active' ? 'Активен' : 'Требует оплаты'}</Badge></div>
      ...
      {loading ? <div className="table-empty-state">Загружаем тариф…</div> : <>
        <div className="developer-billing-summary">
          <article><span><ShieldCheck /></span><div><small>Текущий тариф</small><strong>{subscription?.name ?? 'Не подключён'}</strong><em>до {date(subscription?.current_period_end)}</em></div><button type="button" onClick={() => void onRenew()} disabled={!subscription || Boolean(processing)}><RefreshCw /> {processing === 'renew' ? 'Продлеваем…' : 'Продлить на 30 дней'}</button></article>
