...
function eventStatus(event: DeveloperBillingEvent) {
  if (event.provider === 'sandbox_local') return 'тестовый доступ без оплаты';
...
export function DeveloperBillingPanel({ plans, subscription, usage, events, paymentDetails, loading, error, feedback, processing, onRetry, onSubmitContract }: Props) {
  ...
  const isDemo = isPaid && Boolean(subscription?.sandbox_active);
  ...
  return (
    <section className="dashboard-panel developer-billing-panel" id="developer-billing">
      ...
      {loading ? <div className="table-empty-state">Загружаем тариф…</div> : <>
        ...
        <div className="developer-billing-history"><div><h3>История подписки</h3><span>{events.length} операций</span></div>{events.length === 0 ? <p>После подачи договора и перевода заявка появится здесь.</p> : events.slice(0, 6).map((event) => <article key={event.id}><span><CreditCard /></span><div><strong>{event.provider === 'sandbox_local' ? 'Тестовая активация' : event.event_type === 'plan_change' ? 'Смена тарифа' : 'Продление подписки'}</strong><small>{event.provider === 'sandbox_local' ? eventStatus(event) : `${event.provider_reference} · ${eventStatus(event)}`}</small></div><p><strong>{event.provider === 'sandbox_local' ? 'Без оплаты' : money(event.amount_uzs)}</strong><small>{date(event.created_at)}</small></p></article>)}</div>
      </>}
