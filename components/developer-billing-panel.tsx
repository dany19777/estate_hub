...
export function DeveloperBillingPanel({ plans, subscription, usage, events, paymentDetails, loading, error, feedback, processing, onRetry, onSubmitContract }: Props) {
  ...
  return (
    <section className="dashboard-panel developer-billing-panel" id="developer-billing">
      <div className="panel-heading"><div><h2>Тариф и договор</h2><p>{pending ? 'Договор и перевод ожидают проверки суперадмина.' : paymentDetails.bankAccount ? `Оплата банковским переводом: ${paymentDetails.bankName}, счёт ${paymentDetails.bankAccount}. После подписания договора укажите номер операции.` : 'Реквизиты компании ещё настраиваются.'}</p></div><Badge className={`billing-subscription-state ${subscription?.status ?? 'past_due'}`}>{subscription?.status === 'trialing' ? 'Пробный период' : subscription?.status === 'active' ? 'Активен' : 'Требует решения администратора'}</Badge></div>
